...
def calculate_february_balance(request):
    ...
    # Оборудование (текущее, так как нет истории)
    equipment = BalanceData.objects.filter(name="Оборудование").aggregate(total=Sum("amount"))["total"] or Decimal(0)
    
    # Кредиты (текущее)
    credit_total = Credit.objects.aggregate(total=Sum("amount"))["total"] or Decimal(0)
    
    # Краткосрочные обязательства (текущее)
    short_total = ShortTermLiability.objects.aggregate(total=Sum("amount"))["total"] or Decimal(0)
    
    # Инвентарь (текущее)
    inventory_total = InventoryItem.objects.aggregate(total=Sum("total"))["total"] or Decimal(0)
    
    # Дебиторы (долги поставщиков по филиалам до даты)
    total_debtors = Decimal(0)
    for branch in Supplier.objects.exclude(branch=None).values_list("branch__id", "branch__name").distinct():
        branch_id, branch_name = branch
        if branch_name not in ["Филиал 1", "Наши ИП"]:
            branch_debt = sum(
                (t.supplier_debt or Decimal(0))
                for t in Transaction.objects.filter(
                    supplier__branch_id=branch_id, paid_amount__gt=0, created_at__lte=target_date
                )
            )
            total_debtors += branch_debt
    
    # Наличные и счета: агрегируем CashFlow до даты
    safe_amount = Decimal(0)
    for account in Account.objects.all():
        cashflow_sum = CashFlow.objects.filter(account=account, created_at__lte=target_date).aggregate(total=Sum('amount'))['total'] or Decimal(0)
        safe_amount += cashflow_sum
    
    # Бонусы до даты
    bonuses = sum((t.bonus_debt or Decimal(0)) for t in Transaction.objects.filter(paid_amount__gt=0, created_at__lte=target_date))
    
    # Выдачи клиентам до даты
    total_remaining = sum((t.client_debt_paid or Decimal(0)) for t in Transaction.objects.filter(paid_amount__gt=0, created_at__lte=target_date))
    
    # Прибыль инвесторам до даты
    transactions_investors = [
        t for t in Transaction.objects.filter(paid_amount__gt=0, created_at__lte=target_date)
        if getattr(t, 'bonus_debt', 0) == 0 and getattr(t, 'client_debt', 0) == 0 and getattr(t, 'profit', 0) > 0
    ]
    cashflows_investors = CashFlow.objects.filter(
        purpose__operation_type=PaymentPurpose.INCOME, created_at__lte=target_date
    ).exclude(purpose__name__in=["Оплата", "Внесение инвестора", "Возврат от поставщиков"])
    
    total_profit = sum(
        (Decimal(str(getattr(t, 'profit', 0) or 0)) - Decimal(str(getattr(t, 'returned_to_investor', 0) or 0)))
        for t in transactions_investors
    ) + sum(
        (Decimal(str(cf.amount or 0)) - Decimal(str(cf.returned_to_investor or 0)))
        for cf in cashflows_investors
    )
    
    # Вложения инвесторов на дату (текущий баланс - операции после даты)
    ...
    assets_total = equipment + inventory_total + total_debtors + safe_amount
    provisional_liabilities = credit_total + short_total + bonuses + total_remaining + total_profit + investors_total + undistributed_profit
    current_capital = assets_total - provisional_liabilities
    
    return JsonResponse({'capital': str(current_capital)})
