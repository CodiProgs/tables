...
def cash_flow_report(request):
    ...
    for t in transactions:
        ...
        if supplier_id not in cost_rows_dict:
            continue
        ...
        month_attr = f"month_{month_num}"
        current_value = getattr(cost_rows_dict[supplier_id], month_attr)
        setattr(cost_rows_dict[supplier_id], month_attr, current_value + supplier_cost)
    ...
    for row in cost_rows:
        for month in months_cost:
            ...
            setattr(
                cost_total_row,
                month_attr,
                getattr(cost_total_row, month_attr) + getattr(row, month_attr)
            )
        cost_total_row.total += row.total
...
from .models import InvestorDebtOperation
from reportlab.pdfbase import pdfmetrics
from reportlab.pdfbase.ttfonts import TTFont
import os

# --- Регистрация кириллических шрифтов (выполняется один раз при загрузке модуля) ---
_FONT_DIR = os.path.join(os.path.dirname(__file__), "fonts")
try:
    # Имена 'CyrillicFont' и 'CyrillicFont-Bold' будут использоваться в p.setFont()
    pdfmetrics.registerFont(TTFont('CyrillicFont', os.path.join(_FONT_DIR, 'DejaVuSans.ttf')))
    pdfmetrics.registerFont(TTFont('CyrillicFont-Bold', os.path.join(_FONT_DIR, 'DejaVuSans-Bold.ttf')))
except Exception:
    # Защита от повторной регистрации при перезагрузке Django dev-сервера
    pass

@login_required
def investor_operations_pdf(request):
    op_type = request.GET.get("type", "deposit")
    if op_type not in ["deposit", "withdrawal"]:
        return HttpResponse("Некорректный тип операции", status=400)

    operations = InvestorDebtOperation.objects.filter(operation_type=op_type).order_by("created_at")

    response = HttpResponse(content_type='application/pdf')
    response['Content-Disposition'] = f'attachment; filename="operations_{op_type}.pdf"'

    p = canvas.Canvas(response, pagesize=A4)
    width, height = A4

    # Используем зарегистрированный шрифт вместо Helvetica
    p.setFont("CyrillicFont-Bold", 14)
    p.drawString(2*cm, height - 2*cm, f"Операции: {'Внесение' if op_type == 'deposit' else 'Забор'}")

    p.setFont("CyrillicFont", 12)
    y = height - 3*cm
    p.drawString(2*cm, y, "Дата")
    p.drawString(8*cm, y, "Сумма")
    y -= 1*cm
    ...
    for op in operations:
        ...
        if y < 2*cm:
            p.showPage()
            p.setFont("CyrillicFont", 12)  # ВАЖНО: повторно установить шрифт!
            y = height - 2*cm
